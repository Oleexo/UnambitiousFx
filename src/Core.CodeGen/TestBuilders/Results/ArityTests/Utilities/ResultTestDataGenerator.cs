...
internal sealed class ResultTestDataGenerator
{
    ...
    public TestData GenerateSuccessData(ushort arity)
    {
        ...
        return new TestData(
            arity: arity,
            values: typeValuePairs,
            expectedState: ResultState.Success,
            setupCode: setupCode
        );
    }
    ...
    public TestData GenerateExceptionData(ushort arity)
    {
        ...
        return new TestData(
            arity: arity,
            values: typeValuePairs,
            expectedState: ResultState.Exception,
            setupCode: setupCode
        );
    }
    ...
    public TestData GenerateEdgeCaseData(ushort arity)
    {
        ...
        return new TestData(
            arity: arity,
            values: typeValuePairs,
            expectedState: ResultState.EdgeCase,
            setupCode: setupCode
        );
    }
    ...
    public TestData GenerateTypeCoverageData(ushort arity, bool useNullableTypes = false, bool useReferenceTypes = false)
    {
    ...
    public TestData GenerateFactoryMethodData(ushort arity, string factoryMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateTransformationData(ushort arity, string transformationMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateValidationData(ushort arity, string validationMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateSideEffectData(ushort arity, string sideEffectMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateErrorHandlingData(ushort arity, string errorHandlingMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateValueAccessData(ushort arity, string valueAccessMethod)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    public TestData GenerateDirectMethodData(ushort arity, string directMethod, bool isAsync = false)
    {
        var typeValuePairs = GenerateTypeValuePairs(arity);
    ...
    private IEnumerable<TypeValuePair> GenerateTypeCoverageTypeValuePairs(ushort arity, bool useNullableTypes, bool useReferenceTypes)
    {
        for (int i = 1; i <= arity; i++)
        {
            string typeName;
    ...
    private string GenerateTypeCoverageSetupCode(ushort arity, bool useNullableTypes, bool useReferenceTypes)
    {
    ...
    private string GenerateFactoryMethodSetupCode(ushort arity, string factoryMethod)
    {
        var values = Enumerable.Range(1, arity)
            .Select(i => TestTypeHelper.GetTestValue(i))
            .ToArray();
    ...
    private string GenerateTransformationSetupCode(ushort arity, string transformationMethod)
    {
        var values = Enumerable.Range(1, arity)
            .Select(i => TestTypeHelper.GetTestValue(i))
            .ToArray();
    ...
    private string GenerateValidationSetupCode(ushort arity, string validationMethod)
    {
        var baseSetup = GenerateSuccessSetupCode(arity);
    ...
    private string GenerateTrySetupCode(ushort arity, string[] values)
    {
        if (arity == 1)
        {
    ...
    private string GenerateSideEffectSetupCode(ushort arity, string sideEffectMethod)
    {
        var baseSetup = GenerateSuccessSetupCode(arity);
    ...
    private string GenerateErrorHandlingSetupCode(ushort arity, string errorHandlingMethod)
    {
    ...
    private string GenerateValueAccessSetupCode(ushort arity, string valueAccessMethod)
    {
        var baseSetup = GenerateSuccessSetupCode(arity);
    ...
    private string GenerateDirectMethodSetupCode(ushort arity, string directMethod, bool isAsync)
    {
        var baseSetup = GenerateSuccessSetupCode(arity);
    ...
    private string GenerateIfSuccessSetupCode(ushort arity, bool isAsync)
    {
        var asyncModifier = isAsync ? "async " : "";
        var taskType = isAsync ? "Task" : "";
        var awaitKeyword = isAsync ? "await " : "";

        if (arity == 1)
        {
            return $"// When: Executing action on success\nvar executed = false;\nFunc<{TestTypeHelper.GetTestType(1)}, {taskType}> action = {asyncModifier}x => {{ executed = true; {awaitKeyword}Task.CompletedTask; }};";
        }
    ...
    private string GenerateIfFailureSetupCode(ushort arity, bool isAsync)
    {
        var asyncModifier = isAsync ? "async " : "";
        var taskType = isAsync ? "Task" : "";
        var awaitKeyword = isAsync ? "await " : "";

        return $"// When: Executing action on failure\nvar executed = false;\nFunc<Exception, {taskType}> action = {asyncModifier}ex => {{ executed = true; {awaitKeyword}Task.CompletedTask; }};";
    }

    private string GenerateTryGetSetupCode(ushort arity, bool isAsync)
    {
        if (arity == 1)
        {
