...
internal sealed class GherkinTestMethodBuilder : IGherkinTestBuilder
{
    ...
    private string GenerateTestName(string methodName, ushort arity, TestScenario scenario)
    {
    ...
    private string GetExpectedBehavior(string methodName, TestScenario scenario)
    {
        return methodName switch
        {
            "Match" => scenario.Type == TestScenarioCategory.Success ? "ExecuteSuccessAction" : "ExecuteFailureAction",
            "IfSuccess" => scenario.Type == TestScenarioCategory.Success ? "ExecuteAction" : "NotExecuteAction",
            "IfFailure" => scenario.Type == TestScenarioCategory.Failure ? "ExecuteAction" : "NotExecuteAction",
            "TryGet" => scenario.Type == TestScenarioCategory.Success ? "ReturnTrueWithValues" : "ReturnFalse",
            "Map" => scenario.Type == TestScenarioCategory.Success ? "TransformValues" : "PreserveFailure",
            "Bind" => scenario.Type == TestScenarioCategory.Success ? "FlattenResult" : "PreserveFailure",
            "Ensure" => scenario.Type == TestScenarioCategory.Success ? "PassValidation" : "FailValidation",
            "Tap" => "ExecuteSideEffect",
            "ValueOr" => scenario.Type == TestScenarioCategory.Success ? "ReturnValues" : "ReturnDefaultValues",
            _ => "BehaveCorrectly"
        };
    }

    private string GenerateMethodCall(string methodName, ushort arity, TestScenario scenario)
    {
        return methodName switch
        {
    ...
    {
        var parameters = string.Join(", ", Enumerable.Range(1, arity).Select(i => $"value{i}"));
        return $"result.Map(({parameters}) => ({parameters}))";
    }

    private string GenerateValueOrCall(ushort arity)
    {
        var defaultValues = string.Join(", ", Enumerable.Range(1, arity).Select(i => $"default{i}"));
    ...
    private string CombineGherkinSections(string givenSection, string whenSection, string thenSection)
    {
        var sb = new StringBuilder();
