...
internal sealed class ThenMethodBuilder
{
    ...
    public MethodWriter BuildStandaloneMethod(ushort arity)
    {
        ...
        {
            var typeParams = string.Join(", ", Enumerable.Range(1, arity).Select(n => $"T{n}"));
            funcSignature = $"Func<{typeParams}, {resultType}>";
            tryGetParams = string.Join(", ", Enumerable.Range(1, arity).Select(n => $"out var value{n}"));
            thenParams = string.Join(", ", Enumerable.Range(1, arity).Select(n => $"value{n}"));
        }
