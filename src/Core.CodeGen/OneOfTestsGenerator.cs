...
internal sealed class OneOfTestsGenerator : BaseCodeGenerator
{
    ...
    protected override void GenerateForArityRange(ushort numberOfArity, string outputPath)
    {
        ...
        {
            GenerateTestClass(arity, outputPath);
        }
    }

    private void GenerateTestClass(ushort arity, string outputPath)
    {
        var fileWriter = new FileWriter($"{Config.BaseNamespace}.Tests.{Config.ClassName}");
        var classWriter = OneOfTestClassBuilder.Build(arity);
        fileWriter.AddClass(classWriter);
        fileWriter.AddUsing($"{Config.BaseNamespace}.OneOf");

        var fileName = Path.Combine(outputPath, $"{Config.ClassName}{arity}Tests.cs");
...
internal static class OneOfTestClassBuilder
{
    ...
    private static void AddFromPositionTests(ClassWriter classWriter, ushort arity)
    {
        for (ushort i = 1; i <= arity; i++)
        {
            var ordinalName = OrdinalHelper.GetOrdinalName(i);
            var testValue = TestTypeHelper.GetTestValue(i);
            var genericTypes = string.Join(", ", Enumerable.Range(1, arity).Select(TestTypeHelper.GetTestType));
    ...
    private static void AddStoreValueTests(ClassWriter classWriter, ushort arity)
    {
        for (ushort i = 1; i <= arity; i++)
        {
            var ordinalName = OrdinalHelper.GetOrdinalName(i);
            ...
            if (TestTypeHelper.IsValueTestable(i))
            {
                classWriter.AddMethod(new MethodWriter(
                    ...
                    body: body.TrimEnd(),
                    visibility: Visibility.Public,
                    attributes: GenerateInlineData(i).Concat([new AttributeReference("Theory")]),
            ...
            else
            {
                classWriter.AddMethod(new MethodWriter(
                    name: $"From{ordinalName}_ShouldStoreValue",
                    returnType: "void",
                    body: body.Replace("value", testValue).TrimEnd(),
                    visibility: Visibility.Public,
                    attributes: [new AttributeReference("Fact")]
                ));
            }
    ...
    private static void AddMatchWithResponseTests(ClassWriter classWriter, ushort arity)
    {
        for (ushort i = 1; i <= arity; i++)
        {
            var ordinalName = OrdinalHelper.GetOrdinalName(i);
    ...
    private static void AddMatchVoidTests(ClassWriter classWriter, ushort arity)
    {
        for (ushort i = 1; i <= arity; i++)
        {
            var ordinalName = OrdinalHelper.GetOrdinalName(i);
    ...
    private static void AddImplicitConversionTests(ClassWriter classWriter, ushort arity)
    {
        var allTypeParams = string.Join(", ", Enumerable.Range(1, arity).Select(TestTypeHelper.GetTestType));
