...
public static class EitherAssertExtensions {
    ...
    public static Task<Either<TLeft, TRight>> ShouldBeRight<TLeft, TRight>(this Task<Either<TLeft, TRight>> awaitable,
        ...
        where TLeft : notnull
        where TRight : notnull {
        var e = awaitable.GetAwaiter().GetResult();
    ...
    public static Either<TLeft, TRight> ShouldBeLeft<TLeft, TRight>(this Either<TLeft, TRight> either,
        ...
        where TLeft : notnull
        where TRight : notnull {
        if (!either.Left(out var l, out _)) Assert.Fail(because);
        left = l;
        return either;
    }

    /// <summary>
    /// Asserts that the Either is Right, extracts the Right value, and provides a custom failure message if the assertion fails.
    /// </summary>
    ...
    public static Either<TLeft, TRight> ShouldBeRight<TLeft, TRight>(this Either<TLeft, TRight> either,
        ...
        where TLeft : notnull
        where TRight : notnull {
        if (!either.Right(out _, out var r)) Assert.Fail(because);
        right = r;
        return either;
    }
    ...
    public static Either<TLeft, TRight> ShouldBeLeftWhere<TLeft, TRight>(this Either<TLeft, TRight> either,
        ...
        if (!either.Left(out var l, out _)) Assert.Fail(because ?? "Expected Either.Left but was Right.");
        if (!predicate(l)) Assert.Fail(because                 ?? $"Left value '{l}' does not satisfy predicate.");
        return either;
    }
    ...
    public static Either<TLeft, TRight> ShouldBeRightWhere<TLeft, TRight>(this Either<TLeft, TRight> either,
        ...
        where TRight : notnull {
        ArgumentNullException.ThrowIfNull(predicate);
        if (!either.Right(out _, out var r)) Assert.Fail(because ?? "Expected Either.Right but was Left.");
        if (!predicate(r)) Assert.Fail(because                  ?? $"Right value '{r}' does not satisfy predicate.");
        return either;
    }
