...
/// <summary>Represents a fluent assertion mechanism for successfully concluded operations, encapsulating a non-nullable value.</summary>
[DebuggerStepThrough]
public readonly struct SuccessAssertion<TValue>
    ...
    public SuccessAssertion<TOut> Map<TOut>(Func<TValue, TOut> map)
        where TOut : notnull => new(map(_value));

    /// <summary>Projects the value to another value while keeping fluent context.</summary>
    ...
    public Result<TOut> ToResult<TOut>(Func<TValue, TOut> projector)
        where TOut : notnull => Result.Success(projector(_value));
    ...
    public void Deconstruct(out TValue value) => value = _value;
}