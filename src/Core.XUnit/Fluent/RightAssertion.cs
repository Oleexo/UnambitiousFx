...
public readonly struct RightAssertion<TLeft, TRight>
    ...
    /// <summary>
    /// Projects the inner value of a <see cref="RightAssertion{TLeft, TRight}"/> into a new value type using the specified projector function.
    /// </summary>
    /// <typeparam name="TOut">The type of the resulting projected value.</typeparam>
    /// <param name="projector">The function to transform the value of <typeparamref name="TRight"/> into a value of <typeparamref name="TOut"/>.</param>
    /// <returns>
    /// A new <see cref="RightAssertion{TLeft, TOut}"/> containing the projected value of type <typeparamref name="TOut"/>.
    /// </returns>
    ...
    public RightAssertion<TLeft, TOut> Map<TOut>(Func<TRight, TOut> projector)
        where TOut : notnull => new(projector(_right));
    ...
    public void Deconstruct(out TRight right) => right = _right;
}