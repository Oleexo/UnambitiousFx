...
internal sealed class RetryResultPolicy : IResultPolicy {
    ...
    public async ValueTask<Result> ExecuteAsync(Func<ValueTask<Result>> action, CancellationToken cancellationToken = default) {
        ...
        for (var attempt = 1; attempt <= _maxAttempts; attempt++) {
            ...
            try {
                ...
                lastResult = result;
                if (_resultPredicate is not null && !_resultPredicate(result)) {
                    return result.WithMetadata("attempts", attempt);
                }
            }
            catch (Exception ex) {
                lastException = ex;
                if (_exceptionPredicate is not null && !_exceptionPredicate(ex)) {
                    return Result.Failure(ex).WithMetadata("attempts", attempt);
                }
            }
            ...
            if (attempt < _maxAttempts) {
                ...
                if (delay > TimeSpan.Zero) {
                    try { await Task.Delay(delay, cancellationToken); } catch (TaskCanceledException) { throw; }
                }
            }
        }

        if (lastResult is not null) return lastResult.WithMetadata("attempts", _maxAttempts);
        if (lastException is not null) return Result.Failure(lastException).WithMetadata("attempts", _maxAttempts);
        // Should not happen: no result and no exception
        return Result.Failure(new Exception("Retry policy produced no result.")).WithMetadata("attempts", _maxAttempts);
    }

    public async ValueTask<Result<T>> ExecuteAsync<T>(Func<ValueTask<Result<T>>> action, CancellationToken cancellationToken = default) where T : notnull {
        if (action is null) throw new ArgumentNullException(nameof(action));
        ...
        for (var attempt = 1; attempt <= _maxAttempts; attempt++) {
            ...
            try {
                ...
                lastException = ex;
                if (_exceptionPredicate is not null && !_exceptionPredicate(ex)) {
                    return Result.Failure<T>(ex).WithMetadata("attempts", attempt);
                }
            }
        ...
        if (lastResult is not null) return lastResult.WithMetadata("attempts", _maxAttempts);
        if (lastException is not null) return Result.Failure<T>(lastException).WithMetadata("attempts", _maxAttempts);
        return Result.Failure<T>(new Exception("Retry policy produced no result.")).WithMetadata("attempts", _maxAttempts);
    }
}

public static partial class ResultPolicies { // make partial to extend in timeout file
