...
public abstract partial class Result : BaseResult {
    ...
    public override string ToString() {
        ...
        if (chosen is ExceptionalError exErr) {
            ...
            headerType = ex.GetType().Name;
            headerMessage = ex.Message;
        }
        else if (chosen is not null) {
            headerType = chosen.GetType().Name;
        ...
        var metaFailurePart = Metadata.Count == 0 ? string.Empty : " meta=" + string.Join(",", Metadata.Take(2).Select(kv => kv.Key + ":" + (kv.Value ?? "null")));
