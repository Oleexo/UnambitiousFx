...
internal sealed class FailureResult<TValue1> : Result<TValue1>IFailureResult
    where TValue1 : notnull
{
    ...
    public override void Match(Action<TValue1> success, Action<Exception> failure) {
        failure(PrimaryException);
    }
    
    public override TOut Match<TOut>(Func<TValue1, TOut> success, Func<Exception, TOut> failure) {
        return failure(PrimaryException);
    }
    ...
    public override bool Ok([NotNullWhen(true)] out TValue1? value1, [NotNullWhen(false)] out Exception? error) {
        value1 = default;
        error = PrimaryException;
        return false;
    }
    ...
    public override void Deconstruct(out bool isSuccess, out TValue1? value, out Exception? error) {
        isSuccess = false;
        value = null;
        error = PrimaryException;
    }
