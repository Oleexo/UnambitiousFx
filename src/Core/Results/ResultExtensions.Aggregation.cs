...
public static class ResultAggregationExtensions {
    /// <summary>
    /// Enumerates all error reasons (IError) attached to a single result (alias to <see cref="ResultErrorEnumerationExtensions.Errors"/>).
    /// </summary>
    public static IEnumerable<IError> Errors(this Result result) => ((BaseResult)result).Errors();

    /// <summary>
    /// Enumerates all error reasons (IError) across a set of results.
    /// </summary>
    public static IEnumerable<IError> AllErrors(this IEnumerable<BaseResult> results) => results.SelectMany(r => r.Errors());
    ...
    /// </summary>
    public static Result Merge(this IEnumerable<Result> results) => results.Merge(MergeFailureStrategy.AccumulateAll);
    ...
    public static Result Merge(this IEnumerable<Result> results,
        ...
        if (strategy == MergeFailureStrategy.FirstFailure) {
            ...
            foreach (var r in list) {
                foreach (var kv in r.Metadata) metadata[kv.Key] = kv.Value; // last write wins
                if (!r.Ok(out var error)) {
                    Exception primary = r is IFailureResult fr
                                            ? fr.PrimaryException
                                            : error;
                    // Create failure without auto ExceptionalError so original domain error ordering preserved
                    var failure = new FailureResult(primary, attachPrimaryExceptionalReason: false);
                    if (successReasons.Count != 0) failure.WithReasons(successReasons);
                    if (r.Reasons.Count      != 0) failure.WithReasons(r.Reasons);
                    if (metadata.Count       != 0) failure.WithMetadata(metadata);
                    return failure;
                }

                // accumulate reasons from successes
                if (r.Reasons.Count != 0) successReasons.AddRange(r.Reasons.Where(rr => rr is not IError));
            }

            // all success
            var successResult = Result.Success();
            if (successReasons.Count != 0) successResult.WithReasons(successReasons);
            if (metadata.Count       != 0) successResult.WithMetadata(metadata);
        ...
        foreach (var r in list) {
            foreach (var kv in r.Metadata) metadataAll[kv.Key] = kv.Value; // last write wins
            foreach (var reason in r.Reasons) {
                if (reason is IError) errorReasonsAll.Add(reason);
                else successReasonsAll.Add(reason);
            }

            if (!r.Ok(out var error)) {
                if (r is IFailureResult fr) exceptions.Add(fr.PrimaryException);
                else exceptions.Add(error);
            }
        }

        if (exceptions.Count == 0) {
            var mergedSuccess = Result.Success();
            if (successReasonsAll.Count != 0) mergedSuccess.WithReasons(successReasonsAll);
            if (metadataAll.Count       != 0) mergedSuccess.WithMetadata(metadataAll);
            return mergedSuccess;
        }

        Exception primaryAcc = exceptions.Count == 1
                                   ? exceptions[0]
                                   : new AggregateException(exceptions);
        var mergedFailureAcc = Result.Failure(primaryAcc);
        if (successReasonsAll.Count != 0) mergedFailureAcc.WithReasons(successReasonsAll);
        if (errorReasonsAll.Count   != 0) mergedFailureAcc.WithReasons(errorReasonsAll);
        if (metadataAll.Count       != 0) mergedFailureAcc.WithMetadata(metadataAll);
