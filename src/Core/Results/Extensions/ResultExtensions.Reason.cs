...
public static class ResultReasonExtensions {
    ...
    public static T WithReasons<T>(this T               result,
    ...
    public static T WithMetadata<T>(this T  result,
                                    string  key,
                                    object? value)
        where T : BaseResult {
    ...
    public static T WithMetadata<T>(this T                               result,
        ...
        foreach (var kv in metadata) {
            result.AddMetadata(kv.Key, kv.Value);
        }
    ...
    public static T WithSuccess<T>(this T                                result,
                                   string                                message,
        ...
        if (copyMetadata) {
            foreach (var kv in reasonMetadata) {
                result.AddMetadata(kv.Key, kv.Value);
            }
        }
    ...
    public static T WithSuccess<T>(this T   result,
        ...
        result.AddReason(success);
        if (copyMetadata) {
            foreach (var kv in success.Metadata) {
                result.AddMetadata(kv.Key, kv.Value);
            }
        }
    ...
    public static T WithError<T>(this T result,
                                 IError error,
                                 bool   copyMetadata = true)
        where T : BaseResult {
    ...
    public static T WithErrors<T>(this T              result,
                                  IEnumerable<IError> errors,
                                  bool                copyMetadata = true)
        where T : BaseResult {
        foreach (var e in errors) {
            result.WithError(e, copyMetadata);
        }
    ...
    private sealed record InlineError : ErrorBase {
        public InlineError(string                                code,
                           string                                message,
                           Exception?                            exception,
                           IReadOnlyDictionary<string, object?>? metadata)
