...
internal static class EventDispatcherRegistrationFactory
{
        ...
        EventInfo eventInfo)
    {
        ...
        sb.AppendLine("using UnambitiousFx.Core.Results;");
        sb.AppendLine("using UnambitiousFx.Mediator.Transports.Abstractions;");
        sb.AppendLine();
        sb.AppendLine($"namespace {rootNamespace};");
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("///     Generated event dispatcher registrations for NativeAOT compatibility.");
        sb.AppendLine("///     This class provides typed delegates for each event type to avoid reflection during outbox replay.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine($"public sealed class EventDispatcherRegistration : global::{abstractionsNamespace}.IEventDispatcherRegistration");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    ///     Registers dispatcher delegates for all event types in the assembly.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"register\">A callback to register each event type with its dispatcher delegate.</param>");
        ...
        foreach (var eventType in eventInfo.EventTypes)
        {
            ...
            sb.AppendLine($"        register(typeof({globalizedType}), new Func<global::{abstractionsNamespace}.IEvent, global::{abstractionsNamespace}.IEventDispatcher, DistributionMode, CancellationToken, ValueTask<Result>>(");
            sb.AppendLine("            async (@event, dispatcher, distributionMode, ct) =>");
            sb.AppendLine("            {");
            sb.AppendLine($"                var typedEvent = ({globalizedType})@event;");
            sb.AppendLine("                return await dispatcher.DispatchFromOutboxAsync(typedEvent, distributionMode, ct);");
            sb.AppendLine("            }));");
            sb.AppendLine();
        }
        ...
        EventInfo eventInfo)
    {
        ...
        sb.AppendLine($"    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(global::{abstractionsNamespace}.IEventDispatcher))]");
        sb.AppendLine($"    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(global::{abstractionsNamespace}.IEventHandler<>))]");
