...
public sealed class BindTaskTests {
    ...
    public async Task BindAsync_SomeToSome_ReturnsSome() {
        ...
        var result = await TaskOption(option).BindAsync(value => Maybe.Maybe.Some(value.ToString()));
    ...
    public async Task BindAsync_SomeToNone_ReturnsNone() {
        var option = Maybe.Maybe.Some(10);
        var result = await TaskOption(option).BindAsync(_ => Maybe.Maybe.None<string>());
