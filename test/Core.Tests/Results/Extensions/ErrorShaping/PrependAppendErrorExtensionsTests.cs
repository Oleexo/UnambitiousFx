...
public sealed class PrependAppendErrorExtensionsTests {
    ...
    public void AppendError_Arity1_Failure_Appends() {
        ...
        Assert.False(shaped.Ok(out int _, out var err));
    ...
    public void PrependError_PreservesReasonsAndMetadata() {
        var original = Result.Failure(new Exception("err"))
            .WithError(new UnambitiousFx.Core.Results.Reasons.ConflictError("conflict"))
        ...
        Assert.Contains(shaped.Reasons, r => r is UnambitiousFx.Core.Results.Reasons.ConflictError);
    }
