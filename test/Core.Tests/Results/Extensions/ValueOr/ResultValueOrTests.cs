...
namespace UnambitiousFx.Core.Tests.Results.Extensions.ValueOr;
...
public class ResultValueOrTests {
    ...
    [Fact]
    public async System.Threading.Tasks.Task ValueOr_Task_Failure() {
        var task = System.Threading.Tasks.Task.FromResult(Result.Failure<int>(new Exception()));
        var v    = await task.ValueOr(42);
        Assert.Equal(42, v);
    }

    [Fact]
    public async System.Threading.Tasks.Task ValueOr_Task_Success() {
        var task = System.Threading.Tasks.Task.FromResult(Result.Success(11));
        var v    = await task.ValueOr(99);
        Assert.Equal(11, v);
    }

    [Fact]
    public async System.Threading.Tasks.Task ValueOr_ValueTask_Factory() {
