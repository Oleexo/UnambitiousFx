...
public class ResultValueOrMultiArityTests {
    ...
    [Fact]
    public async System.Threading.Tasks.Task ValueOr_Task_Arity2_Failure_ReturnsFallback() {
        var task  = System.Threading.Tasks.Task.FromResult(Result.Failure<int, int>(new Exception()));
        var value = await task.ValueOr((5, 6));
        Assert.Equal((5, 6), value);
    }

    [Fact]
    public async System.Threading.Tasks.Task ValueOr_ValueTask_Arity3_Factory_Failure() {
