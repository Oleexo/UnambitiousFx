// <auto-generated>
//     This code was generated by UnambitiousFx.Core.CodeGen.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>

#nullable enable

using System;
using System.Linq;
using System.Threading.Tasks;
using UnambitiousFx.Core;
using UnambitiousFx.Core.Results;
using UnambitiousFx.Core.Results.Extensions.Validations.Tasks;
using UnambitiousFx.Core.Results.Reasons;
using Xunit;

namespace UnambitiousFx.Core.Tests.Results.Extensions.Validations.Tasks;

public class ResultEnsureTaskTestsArity1
{
    #region Arity 1 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity1_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var taskResult = Task.FromResult(Result.Success(value1));
        Func<int, Task<bool>> predicate = (_) => Task.FromResult(true);
        Func<int, Task<IError>> errorFactory = (_) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity1_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int>("Test error"));
        Func<int, Task<bool>> predicate = (_) => Task.FromResult(false);
        Func<int, Task<IError>> errorFactory = (_) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity1_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var taskResult = Task.FromResult(Result.Success(value1));
        Func<int, Task<bool>> predicate = (_) => Task.FromResult(false);
        Func<int, Task<IError>> errorFactory = (_) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 1 - Task Ensure
    
    #region Arity 2 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity2_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var taskResult = Task.FromResult(Result.Success(value1, value2));
        Func<int, string, Task<bool>> predicate = (_, _) => Task.FromResult(true);
        Func<int, string, Task<IError>> errorFactory = (_, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity2_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string>("Test error"));
        Func<int, string, Task<bool>> predicate = (_, _) => Task.FromResult(false);
        Func<int, string, Task<IError>> errorFactory = (_, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity2_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var taskResult = Task.FromResult(Result.Success(value1, value2));
        Func<int, string, Task<bool>> predicate = (_, _) => Task.FromResult(false);
        Func<int, string, Task<IError>> errorFactory = (_, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 2 - Task Ensure
    
    #region Arity 3 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity3_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3));
        Func<int, string, bool, Task<bool>> predicate = (_, _, _) => Task.FromResult(true);
        Func<int, string, bool, Task<IError>> errorFactory = (_, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity3_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool>("Test error"));
        Func<int, string, bool, Task<bool>> predicate = (_, _, _) => Task.FromResult(false);
        Func<int, string, bool, Task<IError>> errorFactory = (_, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity3_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3));
        Func<int, string, bool, Task<bool>> predicate = (_, _, _) => Task.FromResult(false);
        Func<int, string, bool, Task<IError>> errorFactory = (_, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 3 - Task Ensure
    
    #region Arity 4 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity4_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4));
        Func<int, string, bool, double, Task<bool>> predicate = (_, _, _, _) => Task.FromResult(true);
        Func<int, string, bool, double, Task<IError>> errorFactory = (_, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity4_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool, double>("Test error"));
        Func<int, string, bool, double, Task<bool>> predicate = (_, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, Task<IError>> errorFactory = (_, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity4_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4));
        Func<int, string, bool, double, Task<bool>> predicate = (_, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, Task<IError>> errorFactory = (_, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 4 - Task Ensure
    
    #region Arity 5 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity5_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5));
        Func<int, string, bool, double, long, Task<bool>> predicate = (_, _, _, _, _) => Task.FromResult(true);
        Func<int, string, bool, double, long, Task<IError>> errorFactory = (_, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity5_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool, double, long>("Test error"));
        Func<int, string, bool, double, long, Task<bool>> predicate = (_, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, Task<IError>> errorFactory = (_, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity5_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5));
        Func<int, string, bool, double, long, Task<bool>> predicate = (_, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, Task<IError>> errorFactory = (_, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 5 - Task Ensure
    
    #region Arity 6 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity6_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6));
        Func<int, string, bool, double, long, DateTime, Task<bool>> predicate = (_, _, _, _, _, _) => Task.FromResult(true);
        Func<int, string, bool, double, long, DateTime, Task<IError>> errorFactory = (_, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity6_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool, double, long, DateTime>("Test error"));
        Func<int, string, bool, double, long, DateTime, Task<bool>> predicate = (_, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Task<IError>> errorFactory = (_, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity6_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6));
        Func<int, string, bool, double, long, DateTime, Task<bool>> predicate = (_, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Task<IError>> errorFactory = (_, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 6 - Task Ensure
    
    #region Arity 7 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity7_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var value7 = Guid.NewGuid();
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6, value7));
        Func<int, string, bool, double, long, DateTime, Guid, Task<bool>> predicate = (_, _, _, _, _, _, _) => Task.FromResult(true);
        Func<int, string, bool, double, long, DateTime, Guid, Task<IError>> errorFactory = (_, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity7_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool, double, long, DateTime, Guid>("Test error"));
        Func<int, string, bool, double, long, DateTime, Guid, Task<bool>> predicate = (_, _, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Guid, Task<IError>> errorFactory = (_, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity7_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var value7 = Guid.NewGuid();
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6, value7));
        Func<int, string, bool, double, long, DateTime, Guid, Task<bool>> predicate = (_, _, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Guid, Task<IError>> errorFactory = (_, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 7 - Task Ensure
    
    #region Arity 8 - Task Ensure
    
    [Fact]
    public async Task EnsureTask_Arity8_ValidCondition_ShouldSucceed() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var value7 = Guid.NewGuid();
        var value8 = TimeSpan.FromMinutes(5);
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6, value7, value8));
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<bool>> predicate = (_, _, _, _, _, _, _, _) => Task.FromResult(true);
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<IError>> errorFactory = (_, _, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.True(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity8_FailureResult_ShouldNotValidate() {
        // Given
        var taskResult = Task.FromResult(Result.Failure<int, string, bool, double, long, DateTime, Guid, TimeSpan>("Test error"));
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<bool>> predicate = (_, _, _, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<IError>> errorFactory = (_, _, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    [Fact]
    public async Task EnsureTask_Arity8_InvalidCondition_ShouldFail() {
        // Given
        var value1 = 42;
        var value2 = "test";
        var value3 = true;
        var value4 = 3.14;
        var value5 = 123L;
        var value6 = DateTime.UtcNow;
        var value7 = Guid.NewGuid();
        var value8 = TimeSpan.FromMinutes(5);
        var taskResult = Task.FromResult(Result.Success(value1, value2, value3, value4, value5, value6, value7, value8));
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<bool>> predicate = (_, _, _, _, _, _, _, _) => Task.FromResult(false);
        Func<int, string, bool, double, long, DateTime, Guid, TimeSpan, Task<IError>> errorFactory = (_, _, _, _, _, _, _, _) => Task.FromResult<IError>(new Error("Validation failed"));
        // When
        var ensuredResult = await taskResult.EnsureAsync(predicate, errorFactory);
        // Then
        Assert.False(ensuredResult.IsSuccess);
    }
    
    #endregion // Arity 8 - Task Ensure
}
