...
public sealed class AdvancedResultExtensionsTests {
    ...
    public void MapErrors_Arity0_Success_NoChange() {
        var r = Result.Success().WithMetadata("a", 1);
    ...
    public void TapBoth_Arity1_Success_InvokesSuccessOnly() {
        ...
        int successCalls = 0; int failCalls = 0;
        var tapped = r.TapBoth(v => successCalls = v, _ => failCalls++);
        Assert.True(tapped.Ok(out int value, out _));
    ...
    public void TapBoth_Arity1_Failure_InvokesFailureOnly() {
        ...
        int successCalls = 0; Exception? captured = null;
        var tapped = r.TapBoth(_ => successCalls++, e => captured = e);
        Assert.False(tapped.Ok(out int _, out var err));
        Assert.Equal(ex, err);
    ...
    public void TapBoth_Arity2_Failure_InvokesFailureOnly() {
        ...
        int successCalls = 0; Exception? captured = null;
        var tapped = r.TapBoth((a,b) => successCalls++, e => captured = e);
    ...
    public void TapEither_Alias_Works_Arity2() {
        ...
        int sum = 0; int failCalls = 0;
        var tapped = r.TapEither((a,b) => sum = a + b, _ => failCalls++);
    ...
    public async System.Threading.Tasks.Task TapErrorAsync_NonGeneric_Failure_Taps() {
        ...
        int tapped = 0;
        var after = await r.TapErrorAsync(e => { tapped++; return System.Threading.Tasks.Task.CompletedTask; });
    ...
    public async System.Threading.Tasks.Task MapErrorAsync_NonGeneric_Failure_Maps() {
        ...
        var mapped = await UnambitiousFx.Core.Results.Tasks.ResultExtensions.MapErrorAsync(r, async e => { await System.Threading.Tasks.Task.CompletedTask; return new Exception("mapped", e); });
        Assert.False(mapped.Ok(out _));
        if (!mapped.Ok(out var err)) {
    ...
    public async System.Threading.Tasks.Task MapErrorAsync_NonGeneric_Success_NoChange() {
        var r = Result.Success();
        var mapped = await r.MapErrorAsync(e => System.Threading.Tasks.Task.FromResult<Exception>(new Exception("mapped")));
        Assert.True(mapped.Ok(out _));
    }
    ...
    public async System.Threading.Tasks.Task TapErrorAsync_NonGeneric_ValueTaskVariant() {
        var r = Result.Failure(new Exception("x"));
        int taps = 0;
        var after = await r.TapErrorAsync(e => { taps++; return new ValueTask(); });
        Assert.False(after.Ok(out _));
        Assert.Equal(1, taps);
    }
    ...
    public async System.Threading.Tasks.Task MapErrorAsync_Generic_Arity1_Transforms() {
        ...
        var mapped = await UnambitiousFx.Core.Results.Tasks.ResultExtensions.MapErrorAsync(r, async e => { await System.Threading.Tasks.Task.CompletedTask; return new Exception("e2", e); });
        Assert.False(mapped.Ok(out int _, out var err));
    ...
    public async System.Threading.Tasks.Task TapErrorAsync_Generic_Arity2_Failure() {
        ...
        int taps = 0;
        var mapped = await r.TapErrorAsync(e => { taps++; return System.Threading.Tasks.Task.CompletedTask; });
