...
public sealed class ToArity5BindAsyncValueTaskTests {
    [Fact]
    ...
    public async Task BindAsync_4_To_5_Success() {
        var r = await Result.Success(1, 2, 3, 4)
                             .BindAsync((_, _, _, _) => ValueTask.FromResult(Result.Success(1, 2, 3, 4, 5)));
        r.ShouldBeSuccess(out var v);
        Assert.Equal((1, 2, 3, 4, 5), v);
    }
    ...
    public async Task BindAsync_7_To_5_Success() {
        var r = await Result.Success(1, 2, 3, 4, 5, 6, 7)
                             .BindAsync((_, _, _, _, _, _, _) => ValueTask.FromResult(Result.Success(1, 2, 3, 4, 5)));
        r.ShouldBeSuccess(out var v);
        Assert.Equal((1, 2, 3, 4, 5), v);
    }
