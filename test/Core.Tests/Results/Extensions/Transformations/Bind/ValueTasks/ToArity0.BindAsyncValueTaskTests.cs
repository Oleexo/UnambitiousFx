...
public sealed class ToArity0BindAsyncValueTaskTests {
    [Fact]
    ...
    public async Task BindAsync_4_To_0_Success() {
        var r = await Result.Success(1, 2, 3, 4)
                             .BindAsync((_, _, _, _) => ValueTask.FromResult(Result.Success()));
        r.ShouldBeSuccess();
    }
    ...
    public async Task BindAsync_7_To_0_Success() {
        var r = await Result.Success(1, 2, 3, 4, 5, 6, 7)
                             .BindAsync((_, _, _, _, _, _, _) => ValueTask.FromResult(Result.Success()));
        r.ShouldBeSuccess();
    }
