...
public sealed class ResultAggregationExtensionsTests {
    ...
    public void AllErrors_MixedResults_EnumeratesAllDomainErrors() {
        var r1 = Result.Success().WithSuccess("ok1");
        var r2 = Result.Failure(new ValidationError(new List<string>{"fail-a"}));
        var r3 = Result.Failure(new NotFoundError("User","42"));
        var r4 = Result.Success().WithSuccess("ok2");

        var all = new[] { r1, r2, r3, r4 }.AllErrors().ToList();
    ...
    public void Merge_AllSuccess_PreservesSuccessReasonsAndMetadata() {
        var r1 = Result.Success().WithSuccess("s1").WithMetadata("a",1);
        var r2 = Result.Success().WithSuccess("s2").WithMetadata("b",2);

        var merged = new[] { r1, r2 }.Merge();

        Assert.True(merged.Ok(out _));
        Assert.Equal(2, merged.Reasons.OfType<ISuccess>().Count());
    ...
    public void Merge_MultipleFailures_AggregatesExceptionsAndReasons() {
        ...
        var errorCodes = merged.Errors().Select(e => e.Code).ToList();
    ...
    public void Merge_FirstFailure_StopsEarlyAndDoesNotAccumulateLaterMetadataOrReasons() {
        var first = Result.Success().WithSuccess("s1").WithMetadata("a", 1);
        var failing = Result.Failure(new ValidationError(new List<string>{"boom"})).WithMetadata("b", 2);
        var after = Result.Success().WithSuccess("s2").WithMetadata("c", 3); // should be ignored

        var merged = new[] { first, failing, after }.Merge(ResultAggregationExtensions.MergeFailureStrategy.FirstFailure);

        Assert.False(merged.Ok(out var primary));
        Assert.IsType<ValidationError>(merged.Errors().First());
    ...
    public void Merge_AccumulateAll_CollectsAllMetadataLastWriteWins() {
        var r1 = Result.Success().WithMetadata("k", 1);
        var r2 = Result.Success().WithMetadata("k", 2); // overwrites
        var r3 = Result.Success().WithMetadata("k", 3); // final
