...
public sealed class ResultArity5Tests
{
    [Fact]
    public void Success_IsSuccess_ReturnsTrue() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m);
    ...
    public void Failure_IsSuccess_ReturnsFalse() {
        var r = Result.Failure<int, string, bool, double, decimal>(new Exception("boom"));
    ...
    public void Failure_MatchWithoutResponse_CallsFailureAction() {
        ...
        var called = false;
        r.Match(
            success: (_, _, _, _, _) => Assert.Fail("Expected failure"),
            failure: errors => {
                called = true;
                Assert.Single(errors);
    ...
    public void Failure_IfSuccess_DoesNotCallSuccessAction() {
        var r = Result.Failure<int, string, bool, double, decimal>(new Exception("boom"));
        r.IfSuccess((_, _, _, _, _) => Assert.Fail("Expected failure"));
    ...
    public void Success_Ok_ReturnsValue() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m);
        if (r.TryGet(out var value1, out var value2, out var value3, out var value4, out var value5)) {
    ...
    public void Failure_Ok_ReturnsErrorMessage() {
        ...
        if (!r.TryGet(out _, out _, out _, out _, out _, out var errors)) {
            Assert.NotNull(errors);
            Assert.Single(errors);
            Assert.Equal("boom", errors.First().Message);
        }
        ...
        if (r.TryGet(out _, out _, out _, out _, out _)) {
            Assert.Fail("Expected failure");
        }
