...
public class ResultFlattenTests
{
    ...
    public void Flatten_SingleArity_Success()
    {
        ...
        Assert.True(flat.Ok(out int value));
    ...
    public void Flatten_SingleArity_OuterFailure()
    {
        ...
        Assert.False(flat.Ok(out int _));
    }
    ...
    [Fact]
    public async Task Flatten_Async_Task()
    {
        var inner = Result.Success(10);
        var outer = Result.Success(inner);
        var flat = await Task.FromResult(outer).Flatten();
        Assert.True(flat.IsSuccess);
        Assert.True(flat.Ok(out int v));
        Assert.Equal(10, v);
    }

    [Fact]
    public async Task Flatten_Async_ValueTask()
    {
