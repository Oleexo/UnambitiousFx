...
public sealed class ResultPolicyTests {
    ...
    public async Task Retry_Succeeds_FirstAttempt() {
        ...
        var r = await policy.ExecuteAsync(() => Result.Success(), TestContext.Current.CancellationToken);
    ...
    public async Task Retry_StopsEarly_WhenPredicateBlocks() {
        ...
        var r = await policy.ExecuteAsync(() => { attempts++; return Result.Failure(new Exception("fail")); }, TestContext.Current.CancellationToken);
