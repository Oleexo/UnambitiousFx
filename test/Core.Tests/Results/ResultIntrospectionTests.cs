...
{
    private sealed class CustomException : Exception { public CustomException(string m="custom") : base(m) {} }
    ...
    public void NonGeneric_Deconstruct_Success()
    {
        ...
        Assert.True(ok);
        Assert.Null(err);
        Assert.Equal("Success", r.ToString().Split(' ')[0]);
    }
    ...
    public void SingleArity_ToNullable_SuccessAndFailure()
    {
        ...
        var successValue = ok.ToNullable<int>();
        var failedValue = fail.ToNullable<int>();
        ...
        Assert.Equal(default(int), failedValue);
    ...
    public void MultiArity3_ToNullable_Success()
    {
        ...
        var nullable = r.ToNullable<int,int,int>();
