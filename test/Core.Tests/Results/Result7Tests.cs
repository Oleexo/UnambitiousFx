...
public sealed class ResultArity7Tests
{
    [Fact]
    public void Success_IsSuccess_ReturnsTrue() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m, 1000L, 'x');
    ...
    public void Failure_IsSuccess_ReturnsFalse() {
        var r = Result.Failure<int, string, bool, double, decimal, long, char>(new Exception("boom"));
    ...
    public void Failure_MatchWithoutResponse_CallsFailureAction() {
        ...
        var called = false;
        r.Match(
            success: (_, _, _, _, _, _, _) => Assert.Fail("Expected failure"),
            failure: errors => {
                called = true;
                Assert.Single(errors);
    ...
    public void Success_MatchWithResponse_CallsSuccessAction() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m, 1000L, 'x');
        var e = r.Match(
            success: (v1, v2, v3, v4, v5, v6, v7) => v1,
            failure: _ => {
                Assert.Fail("Expected failure");
                return 0;
            }
    ...
    public void Success_IfSuccess_CallsSuccessAction() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m, 1000L, 'x');
        var isCalled = false;
        r.IfSuccess((v1, v2, v3, v4, v5, v6, v7) => {
            Assert.Equal(42, v1);
            Assert.Equal("foo", v2);
            Assert.True(v3);
            Assert.Equal(3.14, v4);
    ...
    public void Failure_IfSuccess_DoesNotCallSuccessAction() {
        var r = Result.Failure<int, string, bool, double, decimal, long, char>(new Exception("boom"));
        r.IfSuccess((_, _, _, _, _, _, _) => Assert.Fail("Expected failure"));
    ...
    public void Success_Ok_ReturnsValue() {
        var r = Result.Success(42, "foo", true, 3.14, 99.99m, 1000L, 'x');
        if (r.TryGet(out var value1, out var value2, out var value3, out var value4, out var value5, out var value6, out var value7)) {
    ...
    public void Failure_Ok_ReturnsErrorMessage() {
        ...
        if (!r.TryGet(out _, out _, out _, out _, out _, out _, out _, out var errors)) {
            Assert.NotNull(errors);
            Assert.Single(errors);
            Assert.Equal("boom", errors.First().Message);
        }
        ...
        if (r.TryGet(out _, out _, out _, out _, out _, out _, out _)) {
            Assert.Fail("Expected failure");
        }
