...
public sealed class ResultArity5Tests {
    ...
    public void Success_IsSuccess_ReturnsTrue() {
        var r = Result.Success(42, "foo", true, 7, "bar");
    ...
    public void Failure_IsSuccess_ReturnsFalse() {
        var r = Result.Failure<int, string, bool, int, string>(new Exception("boom"));
    ...
    public void Success_MatchWithResponse_CallsSuccessAction() {
        ...
        var e = r.Match(
            success: (v1, v2, v3, v4, v5) => v1 + v2.Length + (v3 ? 1 : 0) + v4 + v5.Length,
            failure: _ => {
                Assert.Fail("Expected success");
                return 0;
            });
    ...
    public void Success_Deconstruct_ReturnsValue() {
        ...
        Assert.True(ok);
        Assert.Equal((42, "foo", true, 7, "bar"), value);
        Assert.Null(err);
    }
    ...
    public void Failure_Ok_ReturnsErrorMessage() {
        ...
        if (!r.TryGet(out _, out _,out _,out _,out _,out var err)) {
            Assert.Equal("boom", err.Message);
        }
        else {
            Assert.Fail("Expected failure");
