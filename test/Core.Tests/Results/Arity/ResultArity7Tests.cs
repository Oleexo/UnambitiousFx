...
public sealed class ResultArity7Tests {
    ...
    public void Success_Ok_ReturnsValue() {
        ...
        if (r.Ok(out var value1, out var value2,out var value3,out var value4,out var value5,out var value6,out var value7)) {
    ...
    public void Failure_Ok_ReturnsErrorMessage() {
        ...
        if (!r.Ok(out _, out _,out _,out _,out _,out _,out _,out var err)) {
            Assert.Equal("boom", err.Message);
        }
        else {
            Assert.Fail("Expected failure");
